...
class AppGUI:
    ...
    def _create_main_frame(self):
        ...
        report_button = tk.Button(self.root, text="Отчетность", command=self._open_reports_window, font=("Arial", 12))
        report_button.pack(padx=10, pady=10, anchor="ne")
        report_button.place(relx=0.98, rely=0.055, anchor="ne")
    ...
    def _create_stock_tab(self):
        ...
        tk.Button(stock_tab, text="Новая запись", command=self._open_add_stock_window, font=("Arial", 12)).pack(pady=10, anchor="se", padx=20, side="right")
    ...
    def _create_orders_tab(self):
        ...
        tk.Button(orders_tab, text="Новая запись", command=self._open_add_order_window, font=("Arial", 12)).pack(pady=10, anchor="se", padx=20, side="right")
    ...
    def _create_sales_tab(self):
        ...
        tk.Button(sales_tab, text="Новая запись", command=self._open_add_sale_window, font=("Arial", 12)).pack(pady=10, anchor="se", padx=20, side="right")
    ...
    def _open_reports_window(self):
        if self.report_window is None or not tk.Toplevel.winfo_exists(self.report_window):
            ...
            reports_window = self.report_window
            reports_window.title("Отчетность")
            reports_window.state('zoomed')
            tk.Label(reports_window, text="ОТЧЕТ О ВЫРУЧКЕ КОМПАНИИ", font=("Arial", 16)).pack(padx=10, pady=10)

            report_frame = ttk.Frame(reports_window)
            report_frame.pack(expand=True, fill='both', padx=20, pady=20)

            columns = ("Доходы", "Сумма, руб.")
            style = ttk.Style()
            style.configure("Treeview.Heading", font=("Arial", 12))
            style.configure("Treeview", font=("Arial", 12))
            ...
            for col in columns:
                ...
                report_tree.column(col, width=200, anchor='center')

            report_tree.pack(expand=True, fill='both', padx=10, pady=10)
            total_revenue = self._update_report_table(report_tree)
            tk.Label(report_frame, text=f"Выручка компании составляет {total_revenue} руб.", font=("Arial", 12)).pack(padx=10, pady=10, anchor="w")
            tk.Button(report_frame, text="Назад к продажам", command=reports_window.destroy, font=("Arial", 12)).pack(padx=10, pady=10, anchor="se", side="right")
    ...
    def _add_order(self, customer_name, product_name, quantity, status, window, error_label):
        ...
        if self.db_manager.execute_query(query, params):
            window.destroy()
            self._update_orders_table()
            self._update_stock_quantity(product_name, quantity, -1)
        else:
            error_label.config(text="Ошибка при добавлении записи")
    def _add_sale(self, order_id, product_name, quantity, total_amount, window, error_label):
        if product_name.lower() not in [row.lower() for row in self._get_products_from_stock()]:
            error_label.config(text="Такого продукта нет на складе!", wraplength=400)
            return
        if not self._is_order_exist(order_id):
             error_label.config(text="Такого заказа не существует", wraplength=400)
             return
        ...
        if selected_item:
            ...
            try:
                if table == "products":
                    ...
                    order_data = self._get_order_data(record_id)
                    if self._is_order_used_in_sales(record_id):
                        ...
                        return
                    if order_data:
                       self._update_stock_quantity(order_data[0][2], order_data[0][3], 1)
    ...
    def _is_order_used_in_sales(self, order_id):
        ...
        result = self.db_manager.fetch_all(query, params)
        return result and result[0][0] > 0
    def _is_order_exist(self, order_id):
    ...
    def _get_products_from_stock(self):
        ...
        return []
    def _update_report_table(self, tree):
    ...
    def _update_stock_quantity(self, product_name, quantity, increase_or_decrease):
       query = f"SELECT quantity FROM products WHERE product_name = %s"
       params = (product_name,)
       result = self.db_manager.fetch_all(query, params)
       if result and len(result) > 0:
          current_quantity = result[0][0]
          new_quantity = current_quantity + (int(quantity) * int(increase_or_decrease))
          query = f"UPDATE products SET quantity = %s WHERE product_name = %s"
          params = (new_quantity, product_name)
          if not self.db_manager.execute_query(query, params):
            tk.messagebox.showerror("Ошибка", "Не удалось обновить количество товара")
          else:
            self._update_stock_table()
       else:
           tk.messagebox.showerror("Ошибка", "Не удалось обновить количество товара")
